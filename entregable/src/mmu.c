...
#define PG_SET_READ_WRITE_PRESENT_AND_USER 0x00000007
...
unsigned int mmu_inicializar_dir_tarea( unsigned int cr3, unsigned int dirFisicaTarea, int x, int y){
  ...
  *pageDirectory = ((unsigned int)(pageTable) | 7);
  pageDirectory++;
  int i = 1 ;
  for(i = 1; i < 1024; i++){
    *pageDirectory = 0;
...
//          inicializar una nueva tabla de paginas.
    ...
    if(!((*PDE) & PG_PRESENT)){
      nuevaPagina = mmu_proxima_pagina_fisica_libre();
      *PDE = (nuevaPagina | PG_SET_READ_WRITE_PRESENT_AND_USER);
      //Seteamos en 0 la nueva pagina
      int i;
      for(i = 0; i < 1024; i++){
        int * punteroANuevaPagina = (int *) nuevaPagina;
...
//          direccion fisica.
    ...
    *PTE = fisica | PG_SET_READ_WRITE_PRESENT_AND_USER;
